...
export const DriverDashboard: React.FC = () => {
  ...
  const fetchDriverState = async () => {
    try {
      ...
      if (rideRes.data.active && rideRes.data.ride) {
        ...
        setActiveRide(null);
        // If no active ride, fetch pending offers
        fetchOffers();
      }
    } catch (err) {
  ...
  useEffect(() => {
    // 1. New Ride Request broadcasted by passenger
  ...
  const handleCompleteTrip = async () => {
    ...
    try {
      ...
      alert(`Trip completed successfully! Earned: $${res.data.payment?.driver_payout?.toFixed(2) || '0.00'}`);
      setActiveRide(null);
      fetchDriverState();
    } catch (err: any) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-dark-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
        <div className="flex items-center space-x-4">
          <div className="w-14 h-14 rounded-2xl bg-dark-800 border-2 border-emerald-500/50 flex items-center justify-center text-emerald-400">
        ...
        <div className="flex items-center space-x-4">
          <div className="text-right">
            <div className="text-xs font-semibold text-slate-300">
              Driver Status: <span className={isOnline ? 'text-emerald-400 font-bold' : 'text-slate-500'}>{isOnline ? 'Online & Receiving' : 'Offline'}</span>
            </div>
            <p className="text-[11px] text-slate-500">
              {isOnline ? 'Ready for incoming ride dispatches' : 'Switch online to accept trips'}
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-5 rounded-2xl bg-dark-900 border border-slate-800 shadow-lg flex items-center space-x-4">
          <div className="w-12 h-12 rounded-xl bg-brand-500/10 text-brand-400 flex items-center justify-center font-black">
            <DollarSign className="w-6 h-6" />
          </div>
          <div>
            <div className="text-xs text-slate-400 font-medium">Total Lifetime Payout</div>
        ...
        <div className="p-5 rounded-2xl bg-dark-900 border border-slate-800 shadow-lg flex items-center space-x-4">
          <div className="w-12 h-12 rounded-xl bg-purple-500/10 text-purple-400 flex items-center justify-center font-black">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7">
          <LiveMap
            ...
            className="h-[500px] w-full rounded-3xl overflow-hidden border border-slate-800 shadow-2xl"
          />
        </div>
          ...
          {activeRide ? (
            ...
            <div className="bg-dark-900 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-5">
              ...
              <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                <div>
                  <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Trip In Action #{activeRide.id}</span>
              ...
              {activeRide.passenger && (
                <div className="p-3.5 rounded-2xl bg-dark-950 border border-slate-800/80 flex items-center justify-between">
                  ...
                  <a
                    ...
                    className="p-2 rounded-xl bg-brand-500/10 text-brand-400 hover:bg-brand-500 hover:text-dark-950 transition"
                  >
              ...
              {/* Estimated Driver Payout (80%) */}
              <div className="p-3.5 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-between">
                <div>
                  <span className="text-[11px] text-emerald-400 font-bold uppercase tracking-wider">Your Payout (80%)</span>
              ...
              <div className="space-y-2 pt-2">
                ...
                {activeRide.status === 'ARRIVED' && (
                  <button
                    ...
                    className="w-full py-3.5 rounded-2xl bg-brand-500 hover:bg-brand-400 text-dark-950 font-black text-xs transition shadow-lg shadow-brand-500/20 flex items-center justify-center space-x-2 cursor-pointer"
                  >
                ...
                {activeRide.status === 'IN_PROGRESS' && (
                  <div className="space-y-2">
                    <button
                      ...
                      className="w-full py-2.5 rounded-xl bg-dark-800 hover:bg-dark-700 text-brand-400 font-bold text-xs border border-brand-500/30 flex items-center justify-center space-x-2 transition"
                    >
                      <Sparkles className="w-3.5 h-3.5" />
                      <span>{isSimulating ? 'Simulating Live Transit...' : 'Simulate Drive Progress on Map'}</span>
                    </button>
            ...
            <div className="bg-dark-900 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                ...
                <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-dark-800 text-slate-400 border border-slate-700">
              ...
              {!isOnline ? (
                ...
                <div className="py-12 text-center text-slate-400 text-xs space-y-2">
                  <div className="w-10 h-10 rounded-full bg-brand-500/10 text-brand-400 flex items-center justify-center mx-auto animate-pulse">
                    <Clock className="w-5 h-5" />
                  </div>
                  <p className="font-bold text-slate-300">Searching for nearby requests...</p>
                  <p className="text-slate-500 text-[11px]">
                    Ride dispatches will appear here instantly when passengers request a ride.
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                  {pendingOffers.map((offer) => (
                    <div
                      key={offer.id}
                      className="p-4 rounded-2xl bg-dark-950 border border-brand-500/40 shadow-lg space-y-3 animate-in slide-in-from-top-2"
                    >
                      ...
                      <div className="flex items-center justify-between text-[11px] text-slate-400 pt-1 border-t border-slate-800">
                        ...
                        <span>Est. Duration: {offer.duration_minutes} min</span>
                      </div>

                      <div className="flex items-center space-x-2 pt-2">
                        <button
                          onClick={() => handleDeclineRide(offer.id)}
                          className="w-1/3 py-2 rounded-xl bg-dark-800 text-slate-400 text-xs font-semibold hover:bg-dark-700"
                        >
                          Decline
                        </button>
                        <button
                          onClick={() => handleAcceptRide(offer.id)}
                          className="w-2/3 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-dark-950 text-xs font-black transition shadow-lg shadow-emerald-500/20"
                        >
                          Accept Ride Offer
                        </button>
                      </div>
      ...
      {showOtpInput && (
        <div className="fixed inset-0 z-50 bg-dark-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-dark-900 border border-slate-800 rounded-3xl w-full max-w-sm p-6 shadow-2xl space-y-4">
            ...
            <p className="text-xs text-slate-400">Ask the passenger for their 4-digit security code to verify and start the trip:</p>
            ...
            <input
              ...
              className="w-full bg-dark-950 border border-slate-800 rounded-xl p-3 text-center text-2xl font-mono font-bold tracking-widest text-brand-400 focus:outline-none focus:border-brand-500"
            />

            <div className="flex items-center space-x-3 pt-2">
              <button
                onClick={() => setShowOtpInput(false)}
                className="w-1/2 py-2.5 rounded-xl bg-dark-800 text-slate-300 text-xs font-semibold hover:bg-dark-700"
              >
                Cancel
              </button>
              <button
                onClick={handleStartTrip}
                className="w-1/2 py-2.5 rounded-xl bg-brand-500 hover:bg-brand-400 text-dark-950 text-xs font-extrabold"
              >
                Verify & Start
              </button>
            </div>
